...
public class MyDB
{
    public static object CommandTy { get; private set; }
        ...
        try
        {
            if (FU_Image.PostedFile != null)
            {
                ...
                if (fileExtention == ".jpeg" || fileExtention == ".jpg" || fileExtention == ".png")
                {
                    ...
                    FU_Image.SaveAs(HttpContext.Current.Server.MapPath("~/pages/news/newspages/" + rename));

                    url = "~/pages/news/newspages/" + rename;
                }
